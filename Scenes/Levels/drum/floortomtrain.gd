extends Node2D
@onready var noise = get_node("./floortom")
...
func _ready() -> void:
	SignalManager.connect("floortomsongdone",songdone)
...
func _process(delta: float) -> void:
	if Input.is_action_just_released("floortom"):
		notePlayed += 1
		noise.play()
		if currNote == null:
			good.hide()
...
func songdone():
	if score >= 0.6*16:
		GameManager.playerInfo["unlocked"]["drumset"][1]["floortom"] = true
...
func _on_rigid_body_2d_body_exited(body: Node) -> void:
	if str(body.name) == "endofsong":
		SignalManager.emit_signal("floortomsongdone")

extends Node2D
@onready var noise = get_node("./snare")
...
func _ready() -> void:
	SignalManager.connect("snaresongdone",songdone)
...
func _process(delta: float) -> void:
	if Input.is_action_just_released("snare"):
		notePlayed += 1
		noise.play()
		if currNote == null:
			good.hide()
...
func _on_rigid_body_2d_body_exited(body: Node) -> void:
	if str(body.name) == "endofsong":
		SignalManager.emit_signal("snaresongdone")
